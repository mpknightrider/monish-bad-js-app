    ...
    // Add the ObjectId import at the top for type safety and validation
    ...
    module.exports = function productReviews () {
      return (req: Request, res: Response, next: NextFunction) => {
        ...
        db.reviews.findOne({ _id: id }).then((review: Review) => {
            ...
            if (!likedBy.includes(user.data.email)) {
              db.reviews.update(
                ...
                () => {
                  ...
                  setTimeout(function () {
                    db.reviews.findOne({ _id: id }).then((review: Review) => { // <-- injection-safe now
                      const likedBy = review.likedBy
          ...
          // ensure unchanged error handler
